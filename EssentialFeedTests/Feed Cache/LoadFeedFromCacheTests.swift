...
class LoadFeedFromCacheTests: XCTestCase {
   ...
   func test_load_deliversNoImagesOnMoreThanSevenDaysOldCache() {
      let feed = uniqueImageFeed()
      let fixedCurrentDate = Date()
      let moreThanSeven = fixedCurrentDate.adding(days: -7).adding(seconds: -1)
      let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
      
      expect(sut, toCompleteWith: .success([])) {
         store.completeRetrieval(with: feed.local, timestamp: moreThanSeven)
      }
   }
   ...
   func test_load_doesNotDeletesCacheOnRetrievalEmptyCache() {
      let (sut, store) = makeSUT()
   ...
   func test_load_doesNotDelieverResultAfterSUTInstanceHasBeenDeallocates() {
